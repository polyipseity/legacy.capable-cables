...
public class BuilderStructure<T extends BuilderStructure<T, V>, V> extends Builder<T, V> {
	...
	public IMutatorImmutablizable<?, ?> mutator =
			castUncheckedUnboxedNonnull(optionalNonnull(peekDefault(KEY_DEFAULT_MUTATOR), identity(),
					() -> MutatorMutable.INSTANCE));
	public ILogging<Logger> logging = castUncheckedUnboxedNonnull(optionalNonnull(peekDefault(KEY_DEFAULT_LOGGING),
