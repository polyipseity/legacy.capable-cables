...
import $group__.annotations.runtime.processors.ExternalToImmutableMethodProcessor;
...
public interface IImmutablizable<I extends IImmutablizable<I>> {
	...
	ConcurrentMap<ExternalToImmutableMethod, MethodHandle> EXTERNAL_METHOD_MAP =
			MapsExtension.MAP_MAKER_MULTI_THREAD_WEAK_KEYS.makeMap();
	...
	static <T> Optional<T> tryToImmutable(@Nullable T o, @Nullable Logger logger) {
		...
		Class<?> oc = o.getClass();
		if (ExternalToImmutableMethodProcessor.INSTANCE.isProcessed()) {
	...
	static <T> T tryToImmutableUnboxed(@Nullable T o, @Nullable Logger logger) {
		return Optionals.unboxOptional(tryToImmutable(o
				, logger));
	}
