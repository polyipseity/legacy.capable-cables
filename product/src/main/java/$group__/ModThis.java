...
import $group__.annotations.Marker;
import $group__.annotations.Property;
import $group__.client.ProxyClient;
import $group__.client.configurations.ModGuiFactoryThis;
import $group__.common.ModContainerNull;
import $group__.common.registrables.items.ItemWrench;
import $group__.proxies.Proxy;
import $group__.proxies.ProxyNull;
...
@Mod(
		...
		name = $group__.Constants.NAME,
		version = $group__.Constants.VERSION,
		dependencies = $group__.Constants.DEPENDENCIES,
		useMetadata = true,
		acceptedMinecraftVersions = $group__.Constants.ACCEPTED_MINECRAFT_VERSIONS,
		certificateFingerprint = $group__.Constants.CERTIFICATE_FINGERPRINT,
		modLanguageAdapter =
				Constants.PACKAGE + '.' + ModThis.CLASS_SIMPLE_NAME + '$' + ModThis.CustomLanguageAdapter.CLASS_SIMPLE_NAME,
		guiFactory = Constants.PACKAGE + '.' + ModGuiFactoryThis.SUBPACKAGE + '.' + ModGuiFactoryThis.CLASS_SIMPLE_NAME,
		updateJSON = $group__.Constants.UPDATE_JSON
)
public enum ModThis {
	/* SECTION enums */
	INSTANCE;
	...
	public static final ModContainer CONTAINER = Singleton.getSingletonInstance(ModContainerNull.class, Globals.LOGGER);
	@SidedProxy(modId = Constants.MOD_ID,
			clientSide = Constants.PACKAGE + '.' + Proxy.SUBPACKAGE + '.' + ProxyClient.CLASS_SIMPLE_NAME,
			serverSide = Constants.PACKAGE + '.' + Proxy.SUBPACKAGE + '.' + ProxyServer.CLASS_SIMPLE_NAME)
	public static final Proxy PROXY = Singleton.getSingletonInstance(ProxyNull.class, Globals.LOGGER);
	...
	private static <M, E> void processEvent(String name, M mod, E event, BiConsumer<? super M, ? super E> process) {
		Globals.LOGGER.info(() -> Loggers.EnumMessages.FACTORY_PARAMETERIZED_MESSAGE.makeMessage("{} started", name));
		Stopwatch stopwatch = Stopwatch.createStarted();
		process.accept(mod, event);
		Globals.LOGGER.info(() -> Loggers.EnumMessages.FACTORY_PARAMETERIZED_MESSAGE.makeMessage("{} ended ({} <- {} ns)", name, stopwatch.stop(),
	...
	public void processEvent(FMLEvent event) {
		EVENT_MAP.getOrDefault(event.getClass(),
				(m, e) -> Globals.LOGGER.info(() -> Loggers.EnumMessages.FACTORY_PARAMETERIZED_MESSAGE.makeMessage("FMLEvent '{}' received, but " +
	...
	@Config(modid = Constants.MOD_ID, name = $group__.Constants.NAME, category = "General")
	@Config.LangKey(Configuration.LANG_KEY_BASE + ".name")
	public enum Configuration {
		;
		@Config.Ignore
		public static final String LANG_KEY_BASE = "configurations." + Constants.MOD_ID;
	...
	public static class CustomLanguageAdapter extends ILanguageAdapter.JavaAdapter {
					...
					if (m != null) {
						...
						if (mps.length == 1) {
							...
							if (mp.key().equals(fc.getSimpleName()) && mp.value().equals(CONTAINER_VALUE)) {
								Throwables.tryRun(() -> f.setAccessible(true), Globals.LOGGER);
								Throwables.consumeIfCaughtThrowable(t -> Globals.LOGGER.warn(() -> Loggers.EnumMessages.SUFFIX_WITH_THROWABLE.makeMessage(Loggers.EnumMessages.REFLECTION_UNABLE_TO_SET_ACCESSIBLE.makeMessage("container field", f, objectClass, true), t)));

								int fMod = f.getModifiers() & ~Modifier.FINAL;
								Throwables.tryRun(() -> FIELD_MODIFIERS_SETTER.invokeExact(f, fMod), Globals.LOGGER);
								Throwables.consumeIfCaughtThrowable(t -> Globals.LOGGER.warn(() -> Loggers.EnumMessages.SUFFIX_WITH_THROWABLE.makeMessage(Loggers.EnumMessages.INVOCATION_UNABLE_TO_INVOKE_METHOD_HANDLE.makeMessage(FIELD_MODIFIERS_SETTER, f.toGenericString(), fMod), t)));

								Throwables.tryRun(() -> f.set(null, container), Globals.LOGGER);
								Throwables.rethrowCaughtThrowableStatic(true);
								break;
							}
						}
					}
							...
							if (mp.key().equals(m.getClass().getSimpleName()) && mp.value().equals(EVENT_PROCESSOR_VALUE)) {
								...
								for (Field f : containerC.getDeclaredFields()) {
									if (ListMultimap.class.isAssignableFrom(f.getType()) && !isMemberStatic(f)) {
										...
										fv =
												castUncheckedUnboxedNonnull(Throwables.tryCall(() -> f.get(container), Globals.LOGGER).orElseThrow(Throwables::rethrowCaughtThrowableStatic));
		...
		public void setProxy(Field target, Class<?> proxyTarget, Object proxy) throws IllegalArgumentException,
			...
			Throwables.tryRun(() -> FIELD_MODIFIERS_SETTER.invokeExact(target, targetMod), Globals.LOGGER);
			Throwables.consumeIfCaughtThrowable(t -> Globals.LOGGER.warn(() -> Loggers.EnumMessages.SUFFIX_WITH_THROWABLE.makeMessage(Loggers.EnumMessages.INVOCATION_UNABLE_TO_INVOKE_METHOD_HANDLE.makeMessage(FIELD_MODIFIERS_SETTER, target.toGenericString(), targetMod), t)));
