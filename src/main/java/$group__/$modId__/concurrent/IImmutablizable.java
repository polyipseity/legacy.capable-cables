...
public interface IImmutablizable<I extends IImmutablizable<I>> {
	...
	LoadingCache<Class<?>, ExternalToImmutableMethod> EXTERNAL_ANNOTATIONS_CACHE = CacheBuilder.newBuilder().initialCapacity(INITIAL_CAPACITY_4).concurrencyLevel(MULTI_THREAD_THREAD_COUNT).expireAfterAccess(CACHE_EXPIRATION_ACCESS_DURATION, CACHE_EXPIRATION_ACCESS_TIME_UNIT).build(new CacheLoader<Class<?>, ExternalToImmutableMethod>() {
		/* SECTION methods */
	...
	static <T> Optional<T> tryToImmutable(@Nullable T o, @Nullable Logger logger) {
		if (o == null) return Optional.empty();
		else if (o instanceof IImmutablizable<?>)
			return Casts.<IImmutablizable<?>>castUnchecked(o).map(IImmutablizable::toImmutable).flatMap(Casts::castUnchecked);

		Class<?> oc = o.getClass();
		if (ExternalToImmutableMethod.ProcessorRuntime.INSTANCE.isProcessed()) {
