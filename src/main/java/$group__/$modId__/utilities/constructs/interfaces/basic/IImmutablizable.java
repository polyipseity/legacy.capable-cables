	...
	static <T> Optional<T> tryToImmutable(@Nullable T o) {
		...
		if (ExternalToImmutableMethod.AnnotationProcessor.INSTANCE.isProcessed()) {
			...
			try {
				...
				LOGGER.warn("Unable to immutablize object '{}' of class '{}' as no to immutable method is obtained, will NOT attempt to immutable again, stacktrace:" + System.lineSeparator() + "{}", o, oc.toGenericString(), getStackTrace(e));
			...
			setAccessibleWithLogging(m, "to-immutable method", m0s ? null : o, oc, true);

			return castUnchecked((m0s ? m.invoke(null, o) : m.invoke(o)).orElseGet(() -> {
				LOGGER.warn("To-immutable method '{}' failed for object '{}' of class '{}', will NOT attempt to immutable again, stacktrace:" + System.lineSeparator() + "{}", m0.toGenericString(), o, oc.toGenericString(), getStackTrace(caughtThrowableStatic() ? getCaughtThrowableUnboxedNonnullStatic() : new_(new NullPointerException())));
				BROKEN_CLASS_SET.add(oc);
				return castUncheckedUnboxed(o);
			}));
		} else {
			LOGGER.debug("Unable to immutable object '{}' of class '{}' as to immutable method annotation is NOT yet processed, will attempt to immutable again, stacktrace:" + System.lineSeparator() + "{}", o, oc.toGenericString(), getStackTrace(newThrowable()));
			return Optional.of(o);
		}
	}
