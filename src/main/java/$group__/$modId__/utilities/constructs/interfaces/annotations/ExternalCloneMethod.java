...
public @interface ExternalCloneMethod {
	...
	enum AnnotationProcessor implements IAnnotationProcessor.IClass.IElement.IMethod<ExternalCloneMethod> {
		...
		public void processMethod(Result<ExternalCloneMethod> result) {
			...
			ICloneable.EXTERNAL_METHOD_MAP.put(a, MethodAdapter.of(m));

			for (Class<?> k : ks) {
				ap = ICloneable.EXTERNAL_ANNOTATIONS_CACHE.getIfPresent(k);
				ICloneable.EXTERNAL_ANNOTATIONS_CACHE.put(k, a);
				if (ap == null)
					LOGGER.debug(getMessage(this, "Registered method '{}' with annotation '{}' for class '{}'"), m.toGenericString(), a, k.toGenericString());
				else
					LOGGER.warn(getMessage(this, "Replaced previous method '{}' with annotation '{}' with method '{}' with annotation '{}' for class '{}'"), ICloneable.EXTERNAL_METHOD_MAP.get(ap).get().orElseThrow(Throwables::unexpected).toGenericString(), ap, m.toGenericString(), a, k.toGenericString());
			}
		}
	}
}
