...
public class GuiRectangle<T extends GuiRectangle<T, N, C, TH>, N extends Number, C extends Color, TH extends ITheme<TH>> extends GuiDrawable<T, N, C, TH> {
	/* SECTION variables */
	...
	public GuiRectangle(Rectangle<?, N> rectangle, IColored<C> colored, IThemed<TH> themed, IMutatorImmutablizable<?, ?> mutator, ILogging<Logger> logging) {
	...
	@Override
	public T toImmutable() { return castUncheckedUnboxedNonnull(isImmutable() ? this : new GuiRectangle<>(this, IMutatorImmutablizable.of(getMutator().toImmutable()))); }
}
