...
public interface IProcessorRuntime<A extends Annotation> {
	...
	static String makeMessage(IProcessorRuntime<?> processor, @Nullable String msg) { return "Processing annotation '" + processor.annotationType() + '\'' + (msg == null || msg.isEmpty() ? STRING_EMPTY : ": " + msg); }
		...
		interface IElement<A extends Annotation, AE extends AnnotatedElement> extends IClass<A> {
			...
			interface IMethod<A extends Annotation> extends IElement<A, Method> {
				...
				default Method findElement(IClass.Result result, @Nullable Logger logger) {
					...
					if (r == null)
						throw rejectArguments(new NoSuchMethodException(makeMessage(this, "No method name '" + mName + "' in class '" + result.clazz.toGenericString() + '\'')), result.thisAsm);
					return r;
				}
