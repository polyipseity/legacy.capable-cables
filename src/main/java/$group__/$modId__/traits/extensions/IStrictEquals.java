...
public interface IStrictEquals {
	...
	LoadingCache<Class<?>, BiFunction<Object, Object, Function<Function<Object, ? extends Boolean>, Boolean>>> FUNCTION_MAP = CacheBuilder.newBuilder().initialCapacity(INITIAL_SIZE_MEDIUM).expireAfterAccess(CACHE_EXPIRATION_ACCESS_DURATION, CACHE_EXPIRATION_ACCESS_TIME_UNIT).concurrencyLevel(MULTI_THREAD_THREAD_COUNT).build(CacheLoader.from(k -> {
		assert k != null;

		ArrayList<BiFunction<Object, Object, Function<Function<Object, ? extends Boolean>, Boolean>>> efs = new ArrayList<>(INITIAL_SIZE_SMALL);
	...
	static <T, O> boolean isEqual(T thisObj, O other, Function<? super O, ? extends Boolean> superEquals) { return FUNCTION_MAP.getUnchecked(thisObj.getClass()).apply(thisObj, other).apply(castUncheckedUnboxedNonnull(superEquals)); }
