...
public interface ICloneable<T> extends Cloneable {
	...
	LoadingCache<Class<?>, ExternalCloneMethod> EXTERNAL_ANNOTATIONS_CACHE = CacheBuilder.newBuilder().initialCapacity(INITIAL_SIZE_LARGE).expireAfterAccess(CACHE_EXPIRATION_ACCESS_DURATION, CACHE_EXPIRATION_ACCESS_TIME_UNIT).concurrencyLevel(MULTI_THREAD_THREAD_COUNT).build(new CacheLoader<Class<?>, ExternalCloneMethod>() {
		/* SECTION methods */
	 ...
	 * @since 0.0.1.0
	 */
	static <T> Optional<T> tryClone(@Nullable T o, Logger logger) {
	 ...
	 */
	@Nullable
	static <T> T tryCloneUnboxed(@Nullable T o, Logger logger) { return unboxOptional(tryClone(o, logger)); }
	 ...
	 * @since 0.0.1.0
	 */
	static <T> T tryCloneUnboxedNonnull(T o, Logger logger) { return tryClone(o, logger).orElseThrow(Throwables::rethrowCaughtThrowableStatic); }


	static <T> T clone(Callable<?> clone, Logger logger) {
