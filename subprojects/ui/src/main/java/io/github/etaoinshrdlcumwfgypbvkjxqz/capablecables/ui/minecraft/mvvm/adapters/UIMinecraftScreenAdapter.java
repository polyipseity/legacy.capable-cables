...
import io.github.etaoinshrdlcumwfgypbvkjxqz.capablecables.ui.minecraft.mvvm.extensions.UIImmutableMinecraftContainerProviderExtension;
...
import io.github.etaoinshrdlcumwfgypbvkjxqz.capablecables.ui.mvvm.UIImmutableContextContainer;
import io.github.etaoinshrdlcumwfgypbvkjxqz.capablecables.ui.mvvm.viewmodels.UIImmutableViewModelContext;
import io.github.etaoinshrdlcumwfgypbvkjxqz.capablecables.ui.mvvm.views.UIImmutableViewContext;
import io.github.etaoinshrdlcumwfgypbvkjxqz.capablecables.ui.structures.UIImmutableKeyboardKeyPressData;
...
public class UIMinecraftScreenAdapter
	...
	@Nullable
	private UIImmutableMouseButtonClickData lastMouseClickData = null;
	...
	public boolean keyPressed(int key, int scanCode, int modifiers) {
		...
		if (getFocus().map(f -> UIEventUtilities.dispatchEvent(
				addEventKeyboard(this, UIEventUtilities.Factory.createEventKeyDown(context.getViewContext(), f,
						new UIImmutableKeyboardKeyPressData(key, scanCode, modifiers))))).orElse(true)) {
	...
	public void removed() {
		...
		GLFW.glfwSetCursor(MinecraftOpenGLUtilities.getWindowHandle(), MemoryUtil.NULL);
		{
			...
			setTargetBeingHoveredByMouse(context, null, new UIImmutableMouseButtonClickData(cp));
	...
	protected void setTargetBeingHoveredByMouse(IUIContextContainer context, @Nullable IUIEventTarget targetBeingHoveredByMouse, IUIMouseButtonClickData data) {
	...
	protected void setLastMouseClickData(IUIContextContainer context, @Nullable UIImmutableMouseButtonClickData newMouseClickData, @Nullable IUIEventTarget target) {
	...
	protected Optional<? extends UIImmutableMouseButtonClickData> getLastMouseClickData() { return Optional.ofNullable(lastMouseClickData); }
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		...
		UIImmutableMouseButtonClickData d = new UIImmutableMouseButtonClickData(cp, button);
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
		...
		UIEventUtilities.dispatchEvent(
				UIEventUtilities.Factory.createEventWheel(false, context.getViewContext(), target, new UIImmutableMouseButtonClickData(cp), target, delta)); // COMMENT nothing to be scrolled
		return true;
	}
	...
	public void mouseMoved(double mouseX, double mouseY) {
		IUIContextContainer context = getContextContainer();
		Point2D cp = new Point2D.Double(mouseX, mouseY);
		UIImmutableMouseButtonClickData d = new UIImmutableMouseButtonClickData(cp);
		UIEventUtilities.dispatchEvent(
