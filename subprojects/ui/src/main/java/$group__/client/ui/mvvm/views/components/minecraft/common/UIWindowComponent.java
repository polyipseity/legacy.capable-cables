...
import $group__.client.ui.mvvm.views.components.minecraft.common.UIWindowComponent.GuiDragInfoWindow.EnumGuiDragType;
...
public class UIWindowComponent
		<M extends IGuiModel<?>, V extends IUIComponent.IGuiView<?, ?>, C extends IUIComponent.IGuiController.IContainer<?, CH>,
				CH extends IUIComponent>
	...
	@Override
	public EnumUIMouseClickResult onMouseClicked(final AffineTransformStack stack, IUIDataMouseButtonClick drag, Point2D mouse, int button) {
	...
	@Override
	public void reshape(UIDOMLikeComponent<?, ?> invoker) { reshape(invoker, s -> {}); }

	@Override
	public void reshape(UIDOMLikeComponent<?, ?> invoker, Consumer<? super S> transformer) {
		transformShape(this, invoker, transformer);
		// TODO resizing logic
	}

	@Override
	public boolean onMouseDragged(final AffineTransformStack stack, IUIDataMouseButtonClick drag, Point2D mouse, int button) {
		if (isBeingDragged()) {
			data.getDragWindow().ifPresent(d -> {
	...
	@Override
	public void onFocusGet(@Nullable UIDOMLikeComponent<?, ?> from) { getParent().orElseThrow(InternalError::new).moveToTop(this); }
	...
	@Override
	public boolean onMouseDragging(final AffineTransformStack stack, IUIDataMouseButtonClick drag, Rectangle2D mouse, int button) {
	...
	public enum CacheGuiWindow {
		;
		public static final Registry.RegistryObject<UIExtensionCache.IType<Rectangle2D, UIWindowComponent<?, ?, ?, ?>>> RECTANGLE_DRAGGABLE =
				UIExtensionCache.Reg.INSTANCE.register(new ResourceLocation(
						...
						(Function<? super ResourceLocation, ? extends UIExtensionCache.IType<Rectangle2D, UIWindowComponent<?, ?, ?, ?>>>)
								k -> new UIExtensionCache.IType.Impl<Rectangle2D, UIWindowComponent<?, ?, ?, ?>>(k) {
									...
									protected void onShapeDescriptorReshape(EventUIShapeDescriptor.Modify event) {
										if (event.getStage() == EnumEventHookStage.POST) {
											event.getShapeDescriptor().getView()
													.flatMap(IUIComponent.IGuiView::getComponent)
													.filter(UIWindowComponent.class::isInstance)
													.ifPresent(c -> invalidate((UIWindowComponent<?, ?, ?, ?>) c));
										}
									}

									@Override
									public Optional<Rectangle2D> get(UIWindowComponent<?, ?, ?, ?> component) {
		...
		public static final Registry.RegistryObject<UIExtensionCache.IType<Rectangle2D, UIWindowComponent<?, ?, ?, ?>>> RECTANGLE_CLICKABLE =
				UIExtensionCache.Reg.INSTANCE.register(new ResourceLocation(
		...
		public static final Registry.RegistryObject<UIExtensionCache.IType<IUIConstraint, UIWindowComponent<?, ?, ?, ?>>> CONSTRAINT =
				UIExtensionCache.Reg.INSTANCE.register(new ResourceLocation(
								NamespaceUtilities.getNamespacePrefixedString(".", "window", "constraint")),
						(Function<? super ResourceLocation, ? extends UIExtensionCache.IType<IUIConstraint, UIWindowComponent<?, ?, ?, ?>>>)
								k -> new UIExtensionCache.IType.Impl<IUIConstraint, UIWindowComponent<?, ?, ?, ?>>(k) {
									{ Bus.FORGE.bus().get().register(this); }
									...
									@Override
									public Optional<IUIConstraint> get(UIWindowComponent<?, ?, ?, ?> component) {
										return UIExtensionCache.TYPE.getValue().get(component).flatMap(cache -> Try.call(() -> cache.getDelegated().get(getKey(),
												() -> {
		...
		private static ResourceLocation getKey(String path) { return new ResourceLocation(UIWindowComponent.class.getSimpleName().toLowerCase(Locale.ROOT), path); }
	}

	@OnlyIn(Dist.CLIENT)
	public static class View<C extends UIWindowComponent<?, ?, ?, ?>, SD extends IShapeDescriptor<? extends Rectangle2D, ?>> extends UIContainerComponent.View {
	...
	@OnlyIn(Dist.CLIENT)
	public static class Data<E extends Events, C extends Data.ColorData> extends UIContainerComponent.Data<E> {
		public C colors;
		@Nullable
