...
@OnlyIn(Dist.CLIENT)
public abstract class UIButtonComponent<S extends ShapeDescriptor<?>, D extends UIButtonComponent.Data<?, ?>> extends UIContainerComponent<S, D> {
	protected boolean keyPressing = false;

	public UIButtonComponent(S shape, D data) { super(shape, data); }
	...
	@Override
	public boolean onMouseDragged(final AffineTransformStack stack, IUIDataMouseButtonClick drag, Point2D mouse, int button) { return super.onMouseDragged(stack, drag, mouse, button) || stack.getDelegated().peek().createTransformedShape(getShapeDescriptor().getShape()).contains(mouse) && onButtonMouseReleased(button); }
