...
import $group__.client.ui.mvvm.views.events.UIEventTarget;
...
@OnlyIn(Dist.CLIENT)
public class UIDOMLikeComponent extends UIEventTarget
	...
	public UIDOMLikeComponent() { addExtension(new UIExtensionCache()); }
	...
	@Override
	public Optional<IUIExtension> getExtension(ResourceLocation key) { return Optional.ofNullable(getExtensions().get(key)); }
	...
	protected boolean setActive0(final AffineTransformStack stack, boolean active) {
		if (!active) {
			...
			getManager().ifPresent(mag -> {
				...
				IUIComponent.getKeyboardKeysBeingPressedForComponent(this).forEach(pk ->
						mag.onKeyboardKeyPress(s, EnumStage.END,
								pk.getData()));
				Point2D cursor = GLUtilities.getCursorPos();
				IUIComponent.getMouseButtonsBeingPressedForComponent(this).forEach(pm ->
						mag.onMouseButtonPress(s, EnumStage.END,
								new IUIDataMouseButtonClick(cursor, pm.getData().getButton())));
			});
		}
		this.active = active;
		return true;
	...
	@Override
	public Optional<IUIComponentManager> getManager() { return CacheUniversal.MANAGER.getValue().get(this); }
	...
	@Override
	public void onParentChange(@Nullable IUIComponentContainer previous, @Nullable IUIComponentContainer next) { parent = new WeakReference<>(next); }
	...
	@Override
	@OverridingMethodsMustInvokeSuper
	public boolean onMouseButtonPress(final AffineTransformStack stack, EnumStage stage, IUIDataMouseButtonClick data) {
	...
	@Override
	@OverridingMethodsMustInvokeSuper
	public boolean onKeyboardKeyPress(final AffineTransformStack stack, EnumStage stage, IUIDataKeyboardKeyPress data) {
	...
	public boolean onKeyboardKeyPress0(final AffineTransformStack stack, EnumStage stage, IUIDataKeyboardKeyPress data) { return false; }

	public boolean onMouseButtonPress0(final AffineTransformStack stack, EnumStage stage, IUIDataMouseButtonClick data) { return false; }
