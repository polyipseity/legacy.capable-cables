...
import $group__.utilities.helpers.specific.Patterns;
	...
	public enum NBTs {
		...
		static {
			...
			Arrays.stream(NBTBase.NBT_TYPES).forEachOrdered(nbt -> {
				...
				nbtTypeLookup.put("NBTTag" + Patterns.LEFT_N_RIGHT_BRACKETS_PATTERN.matcher(WordUtils.capitalizeFully(nbt)).replaceAll(Matcher.quoteReplacement("Array")), nbtTypeLookup.size());
			});
		...
		public static int lookupNBTTypeNonnull(Class<?> clazz) { return lookupNBTType(clazz).orElseThrow(() -> Throwables.rejectArguments(clazz)); }
