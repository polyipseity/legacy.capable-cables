...
public interface IGuiEventListenerImproved extends IGuiEventListener {
	...
	default boolean onMouseDragged(MatrixStack matrix, Rectangle2D mouse, int button) { return false; }
	...
	@OverridingStatus(group = ConstantsGui.GROUP, when = When.NEVER)
	@Deprecated
	default boolean mouseDragged(double mouseX, double mouseY, int button, double mouseXDiff, double mouseYDiff) { return onMouseDragged(new MatrixStack(), new Rectangle2D.Double(mouseX, mouseY, mouseXDiff, mouseYDiff), button); }
