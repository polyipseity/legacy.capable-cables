...
@OnlyIn(CLIENT)
public abstract class GuiComponent<S extends ShapeDescriptor<?>, D extends GuiComponent.Data<?>> implements IRenderableComponent, IGuiEventListenerComponent {
	...
	protected S shapeDescriptor; // COMMENT relative-to-parent shape
	protected List<Runnable> tasks = new ArrayList<>(INITIAL_CAPACITY_SMALL);

	public GuiComponent(S shapeDescriptor, D data) {
	...
	public void writeStencil(AffineTransformStack stack, Point2D mouse, float partialTicks, boolean increment) {
		if (increment)
			GuiUtilities.DrawingUtilities.drawShape(stack.delegated.peek(), getShapeDescriptor().getShape(), true, Color.WHITE, 0);
		else
			GuiUtilities.DrawingUtilities.drawShape(stack.delegated.peek(), getShapeDescriptor().getShape().getBounds2D(), true, Color.BLACK, 0);
	}

	public S getShapeDescriptor() { return shapeDescriptor; }

	public void transformShape(IGuiReshapeHandler<?> handler, GuiComponent<?, ?> invoker, Consumer<? super S> transformer) {
		Shape pre = getShapeDescriptor().getShape();
		transformer.accept(getShapeDescriptor());
		getShapeDescriptor().constrain(ImmutableList.copyOf(data.constraints));
		getShapeDescriptor().constrain(Collections.singletonList(getConstraintMinimum()));
		onReshape(handler, invoker, pre);
	}
	...
	@Override
	public boolean contains(AffineTransformStack stack, Point2D mouse) { return data.isActive() && stack.delegated.peek().createTransformedShape(getShapeDescriptor().getShape()).contains(mouse); }
	...
	public enum CoordinateConverters {
		...
		public static Point2D toNativePoint(GuiComponent<?, ?> component, Point2D point) {
			Point2D p = (Point2D) point.clone();
			p.setLocation(toNativeCoordinate(component, p.getX()), toNativeCoordinate(component, Minecraft.getInstance().getMainWindow().getScaledHeight() - p.getY()));
			return p;
		}
